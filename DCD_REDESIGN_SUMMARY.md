...
struct dc_extent {              /* was cxled_extent */
    ...
    u16 seq_num;                /* 1..n assembly-order index, dense */
};

struct cxl_dc_tag_group {       /* was region_extent */
    struct cxl_dax_region *cxlr_dax;
    uuid_t uuid;
    struct xarray dc_extents;   /* keyed by seq_num */
...
* No aggregate HPA range exists; each `dc_extent` carries its own
  `hpa_range`.
* `group->dc_extents` is keyed by `seq_num`, a 1..n dense
  assembly-order index.  For sharable extents `seq_num` equals the
  device-stamped `shared_extn_seq` (CXL 3.1 Table 8-51); for
  non-sharable extents the device leaves `shared_extn_seq == 0` and
  `cxl_add_pending()` assigns `seq_num` 1..n in event arrival order.
  The dax layer then sees one uniform 1..n dense invariant
...
* `__cxl_dax_add_resource()` now operates on a single `dc_extent`
  (passes its `hpa_range` and `seq_num` to
...
* `struct dax_resource` gains `u16 seq_num` (1..n assembly-order
  index — sharable: device-stamped, non-sharable: host-assigned).
* `dax_region_add_resource()` signature gains a `seq_num`
...
* `uuid_store()` split into:
  ...
  * `uuid_claim_tagged()` — collect every dax_resource whose uuid
    matches the request, sort by `seq_num`, enforce the
