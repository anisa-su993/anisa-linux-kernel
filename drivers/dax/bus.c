		...
		struct dev_dax *dev_dax, resource_size_t size)
{
	...
	if (size == dev_size)
		return 0;
	if (size != 0 && is_sparse(dax_region))
		return -EOPNOTSUPP;
			  ...
			  const char *buf, size_t len)
{
	...
	for (;;) {
		...
		/*
		 * Untagged resources are independent; only consume one per
		 * write.  Tagged resources may span a logical allocation, so
		 * keep claiming every matching one.
		 */
