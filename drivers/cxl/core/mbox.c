...
static void cxl_walk_cel(struct cxl_memdev_state *mds, size_t size, u8 *cel)
{
	...
	mds->dcd_supported = bitmap_full(dcd_cmds, CXL_DCD_ENABLED_MAX);
}
			     ...
			     bool existing)
{
	...
	/*
	 * Every extent failed to stage (the group was allocated for the first
	 * one, which then failed to insert).  Nothing to online: an empty
	 * online_tag_group() frees @tag_group and still returns success.
	 */
	if (xa_empty(&tag_group->dc_extents)) {
		dev_dbg(dev, "Tag %pUb: no extents staged\n", tag);
		rm_tag_group(tag_group);
		return -ENXIO;
	}

	/*
	 * Extents accepted from a previous boot need to be released on
	 * failure.  New extents skip the release response; they only need to
	 * be omitted from the DC add response.
	 */
	tag_group->skip_device_release = !existing;
...
static int cxl_add_pending(struct cxl_memdev_state *mds, bool existing)
{
	...
	while (!list_empty(pending)) {
		...
		first = list_first_entry(pending, struct cxl_extent_list_node,
					 list);
		import_uuid(&tag, first->extent->uuid);
		/*
