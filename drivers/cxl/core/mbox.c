...
static void cxl_walk_cel(struct cxl_memdev_state *mds, size_t size, u8 *cel)
{
	...
	/*
	 * DCD event handling is not wired up yet; detect command support but
	 * leave the capability disabled so no dynamic capacity is enabled.
	 */
	if (cxl_verify_dcd_cmds(dcd_cmds))
		dev_dbg(dev, "DCD commands present; capability held disabled\n");
